...
func UserAccountRBACMiddleware() echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			conn := db.GetDB(c)
			userAccountRoles, err := db.ReadUserAccountRoles(conn, GetUserAccountID(c))
			if err != nil {
...
func PhaddergruppRBACMiddleware() echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			conn := db.GetDB(c)
			phaddergruppRole, err := db.ReadPhaddergruppRole(conn, GetUserAccountID(c), phaddergruppID)
