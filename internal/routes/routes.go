...
func RegisterRoutes(e *echo.Echo, ss *auth.SessionStore, rts *auth.PasswordResetTokenStore, sender email.Sender) {
	...
	phaddergrupp.POST("/purchase-mums", handlers.PostPhaddergruppPurchaseMums, handlers.PauseGuard())
	phaddergrupp.POST("/mumsa", handlers.PostPhaddergruppMumsa, handlers.PauseGuard())
	...
	phaddergrupp.POST("/pause", handlers.PostPhaddergruppPause, auth.RequirePhaddergruppRole(roles.Phadder))
	phaddergrupp.POST("/unpause", handlers.PostPhaddergruppUnpause, auth.RequirePhaddergruppRole(roles.Phadder))
