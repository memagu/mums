...
type PhaddergruppData struct {
	...
	MumsPauseDuration           time.Duration
}
...
func CreatePhaddergrupp(dbtx DBTX, name, swishRecipientNumber string) (int64, error) {
	res, err := dbtx.Exec(
		...
		config.Defaults.Mums.PauseDuration,
	)
	if err != nil {
		return 0, err
	}
...
func UpdatePhaddergrupp(dbtx DBTX, phaddergruppID int64, phaddergruppData PhaddergruppData) error {
	...
	result, err := dbtx.Exec(sqlQuery,
		...
		phaddergruppData.MumsPauseDuration,
		phaddergruppID,
	)
